...
- **Account deletion must cascade** to all of a user's data (`category`, `sub_tally`, `field_def`, `tag`, `entry`, `entry_tag`, `entry_value`, `match`, `level_rule`, memos) — a deletion that leaves orphaned data is a privacy failure. Enforce via `ON DELETE CASCADE` from `user`.
