...
- **Account deletion must cascade** to all of a user's data (`category`, `activity`, `field_def`, `tag`, `entry`, `entry_tag`, `entry_value`, `match`, `level_rule`, `connection`, `block`, `comment`, memos) — a deletion that leaves orphaned data is a privacy failure. Enforce via `ON DELETE CASCADE` from `user`.
...
- **`private` no longer means hidden.** A private account's `/@{username}` character sheet (activity names + levels/progress) is visible to any visitor and surfaces in search; only entries + free-text notes stay gated. Any change that *widens* what a non-fellow sees on a private account requires fresh consent + a privacy-policy update.
